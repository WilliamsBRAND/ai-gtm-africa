...
import {ChevronLeft, ChevronRight, Menu, X, Mic, Armchair, Map, Lightbulb, Network, ArrowRight, ArrowUpRight} from 'lucide-react';
import './styles.css';
import TomidePage from './TomidePage';
import KigaliPage from './KigaliPage';
...
const cities = [
  ['KIGALI','08 OCTOBER 2026','/images/hall.jpg','center 22%'],
  ['NAIROBI','07 NOVEMBER 2026','/images/audience.jpg','center 38%'],
  ['LAGOS','28 NOVEMBER 2026','/images/hero-conference.png','center 50%'],
...
function App(){
  ...
  // Reset scrolls to 0 on initial mount
  useEffect(()=>{
    if(fiveCitiesRef.current) fiveCitiesRef.current.scrollLeft = 0;
    if(programmeRef.current) programmeRef.current.scrollLeft = 0;
  },[]);
  ...
  return <><Header onRegister={()=>openRegistration()}/>{loading&&<div className="loader" aria-hidden="true"><span>AI GTM AFRICA<i/></span></div>}<main>
    ...
    <section id="five-cities" className="five-cities light">
      ...
      <div className="five-cities-track reveal" ref={fiveCitiesRef} onScroll={onFiveCitiesScroll} tabIndex="0" aria-label="Five event cities, horizontal slider">{fiveCities.map(([name,country,img],i)=><figure key={name} className="five-city"><img src={img} alt={`${name}, ${country} skyline`}/><figcaption><span className="five-city-index">0{i+1}</span><div><span className="five-city-name">{name}</span><span className="five-city-country">{country}</span></div><a href={name==='KIGALI'?'/kigali':'#cities'} className="five-city-see">{name==='KIGALI'?'See Dates &amp; Details':'See Cities &amp; Dates'} <span aria-hidden="true" className="cta-arrow"/></a></figcaption></figure>)}</div>
    </section>
    ...
    <section id="cities" className="cities dark"><div className="cities-bg" key={`bg-${city}`}><img src={cities[city][2]} style={{objectPosition:cities[city][3]}} alt={`${cities[city][0]} conference`}/></div><div className="cities-content reveal"><p className="kicker reveal">2026 CITIES &amp; DATES</p><h2 className="reveal" style={{transitionDelay:'60ms'}}>FIVE CITIES.<br/>ONE GROWING COMMUNITY.</h2><div className="city-switcher reveal" style={{transitionDelay:'120ms'}}><button aria-label="Previous city" disabled={city===0} onClick={()=>setCity((city+4)%5)}><ChevronLeft/></button><div><span>0{city+1} / 05</span><h3>{cities[city][0]}</h3><p>{cities[city][1]}</p></div><button aria-label="Next city" disabled={city===4} onClick={()=>setCity((city+1)%5)}><ChevronRight/></button></div><div className="reveal" style={{transitionDelay:'180ms',display:'flex',gap:'12px',flexWrap:'wrap',alignItems:'center'}}><CTA onClick={()=>openRegistration(city)}>Register Interest in {cities[city][0]}</CTA>{cities[city][0]==='KIGALI'&&<a className="cta cta-dark" href="/kigali">View Kigali Concept Note <span aria-hidden="true" className="cta-arrow"/></a>}</div></div></section>

    <section className="final-cta light"><div className="reveal"><p className="kicker reveal">JOIN US</p><h2 className="reveal" style={{transitionDelay:'60ms'}}>BUILD. GROW. SCALE.</h2><p className="reveal" style={{transitionDelay:'120ms'}}>If you are building, marketing, launching, growing or scaling a business, come be part of the conversation.</p><div className="final-cta-buttons reveal" style={{transitionDelay:'180ms'}}><CTA onClick={()=>openRegistration()}>Register Your Interest</CTA><CTA dark href={WHATSAPP_URL}>Become a Partner</CTA></div></div></section>

  </main><footer id="footer" className="footer dark"><div className="footer-brand reveal"><a className="brand" href="#top">AI GTM <b>AFRICA</b></a><p>AI. GO-TO-MARKET. GROWTH.</p></div><div className="footer-group reveal" style={{transitionDelay:'70ms'}}><p className="footer-label">EXPLORE</p>{nav.map(([n,h])=><a key={n} href={h}>{n}</a>)}<a href="/kigali" style={{color:'#c99398',fontWeight:'600'}}>Kigali Concept Note</a></div><div className="footer-group reveal" style={{transitionDelay:'140ms'}}><p className="footer-label">ATTEND &amp; PARTNER</p><button type="button" className="footer-link" onClick={()=>openRegistration()}>Register Your Interest <ArrowRight size={14} aria-hidden="true"/></button><a className="footer-link" href={WHATSAPP_URL} target="_blank" rel="noreferrer">Become a Partner <ArrowUpRight size={14} aria-hidden="true"/></a><a className="footer-link" href={MEDIA_WHATSAPP_URL} target="_blank" rel="noreferrer">Photography / Media Partnership <ArrowUpRight size={14} aria-hidden="true"/></a></div><div className="footer-group reveal" style={{transitionDelay:'210ms'}}><p className="footer-label">FOLLOW</p><div className="footer-social-row"><a className="footer-social-btn" href="https://www.linkedin.com/in/tomidewill/" target="_blank" rel="noreferrer" aria-label="LinkedIn"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg></a><a className="footer-social-btn" href="https://www.instagram.com/tomidewilliams.ai/" target="_blank" rel="noreferrer" aria-label="Instagram"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect width="20" height="20" x="2" y="2" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" x2="17.51" y1="6.5" y2="6.5"/></svg></a><a className="footer-social-btn" href="https://x.com/tomidewilliams" target="_blank" rel="noreferrer" aria-label="X"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a></div></div><p className="footer-copyright reveal" style={{transitionDelay:'280ms'}}>© 2026 AI GTM Africa</p></footer><ApplicationForm form={form} onClose={()=>setForm(null)}/></>
}
...
const isKigaliPage =
  hostname === 'kigali.aigtm.africa' ||
  hostname === 'kigali.aigtmafrica.xyz' ||
  hostname.startsWith('kigali.') ||
  pathname === '/kigali' ||
  pathname === '/kigali-event' ||
  pathname === '/kigali.html';

const isTomidePage = !isKigaliPage && pathname === '/tomide-williams';

if (isKigaliPage) {
  document.title = 'AI GTM Africa Kigali · 08 October 2026 | Concept Note';
} else if (isTomidePage) {
  document.title = 'Tomide Williams | AI GTM Africa';
} else {
  document.title = 'AI GTM Africa 2026';
}

createRoot(document.getElementById('root')).render(
  isKigaliPage ? <KigaliPage /> : isTomidePage ? <TomidePage /> : <App />
);
